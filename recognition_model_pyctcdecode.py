...
def test(model, testset, device):
    ...
    with torch.no_grad():
        for example in tqdm.tqdm(dataloader, 'Evaluate', disable=None):
            ...
            pred = F.log_softmax(model(X, X_raw, sess), -1)  # Shape: (1, time_steps, num_classes)
...
def train_model(trainset, devset, device, n_epochs=FLAGS.num_epochs):
    ...
    model.load_state_dict(
        torch.load(os.path.join(FLAGS.output_directory, 'model.pt'), map_location=torch.device(device))
    )
    return model
...
def main():
    ...
    logging.info(
        subprocess.run(['git', 'rev-parse', 'HEAD'], stdout=subprocess.PIPE, universal_newlines=True).stdout
    )
    logging.info(
        subprocess.run(['git', 'diff'], stdout=subprocess.PIPE, universal_newlines=True).stdout
    )
